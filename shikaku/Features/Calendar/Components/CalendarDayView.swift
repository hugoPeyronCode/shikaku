...
struct CalendarDayView: View {
    ...
    private func calculateLevelProgress(_ level: ShikakuLevel) -> Double {
        // For JSON-based levels, use the date and level ID to create a deterministic progress
        let calendar = Calendar.current
        let daysSinceLevel = calendar.dateComponents([.day], from: level.date, to: Date()).day ?? 0

        // Create a deterministic but varied progress based on the level's properties
        let levelHash = level.levelDataId.hashValue
        let baseProgress = min(Double(abs(daysSinceLevel)) * 0.05, 0.6)
        let progressSeed = Double(abs(levelHash) % 100) / 100.0 * 0.4

        // Ensure progress is always between 0.1 and 0.9 for visual appeal
        let totalProgress = baseProgress + progressSeed
        return max(0.1, min(totalProgress, 0.9))
    }
}
